...
- **Units (km / mi):** every distance/elevation is wrapped in
  ...
  Both unit strings are stored verbatim (WTA-exact) — JS never does live math, avoiding rounding
  drift. When adding a measurement, always wrap it in a `span.u` with both attributes.
